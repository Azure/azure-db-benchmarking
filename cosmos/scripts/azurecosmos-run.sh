...
if [ ! -z "$userAssignedIdentityClientId" ]; then
   sed -i "s/^[#]*\s*azurecosmos.userAssignedIdentityClientId\ =.*/azurecosmos.userAssignedIdentityClientId\ =\ $userAssignedIdentityClientId/" azurecosmos.properties
fi
