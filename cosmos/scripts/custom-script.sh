...
# When USE_MANAGED_IDENTITY_FOR_STORAGE is set to true, the VM must have a user-assigned managed identity with the following RBAC roles:
#   - Storage Blob Data Contributor (for blob operations)
#   - Storage Table Data Contributor (for table operations)
# These roles should be assigned at the storage account level or higher.
...
if [[ $USE_MANAGED_IDENTITY_FOR_STORAGE == true ]]; then
  ...
  echo "########## Using Connection String for Azure Storage authentication ##########"
  AUTH_MODE="--connection-string $RESULT_STORAGE_CONNECTION_STRING"
  ACCOUNT_NAME_PARAM=""
fi

if [ $MACHINE_INDEX -eq 1 ]; then
  table_exist=$(az storage table exists --name "${benchmarkname}Metadata" $AUTH_MODE $ACCOUNT_NAME_PARAM | jq '.exists')
  if [ "$table_exist" = true ]; then
    echo "${benchmarkname}Metadata already exists"
  else
    az storage table create --name "${benchmarkname}Metadata" $AUTH_MODE $ACCOUNT_NAME_PARAM
  fi
  ...
  az storage container create -n $results_container_name $AUTH_MODE $ACCOUNT_NAME_PARAM

  # Generate result storage URL
  if [[ $USE_MANAGED_IDENTITY_FOR_STORAGE == true ]]; then
    # For managed identity, use the blob endpoint without SAS
    result_storage_url="https://${STORAGE_ACCOUNT_NAME}.blob.core.windows.net/${results_container_name}"
  else
    # For connection string, generate SAS token
    sas=$(az storage container generate-sas -n $results_container_name $AUTH_MODE --https-only --permissions dlrw --expiry $end -o tsv)
  ...
  latest_table_entry=$(az storage entity insert --entity PartitionKey="${tool_api}" RowKey="${GUID}" JobStartTime=$job_start_time JobFinishTime="" JobStatus="Started" NoOfClientsCompleted=0 NoOfClientsStarted=1 SAS_URL=$result_storage_url --table-name "${benchmarkname}Metadata" $AUTH_MODE $ACCOUNT_NAME_PARAM)
  ...
  for i in $(seq 1 10); do
    latest_table_entry=$(az storage entity show --table-name "${benchmarkname}Metadata" $AUTH_MODE $ACCOUNT_NAME_PARAM --partition-key "${tool_api}" --row-key "${GUID}")
    if [ -z "$latest_table_entry" ]; then
  ...
  for j in $(seq 1 60); do
    ...
    replace_entry_result=$(az storage entity merge --table-name "${benchmarkname}Metadata" $AUTH_MODE $ACCOUNT_NAME_PARAM --entity PartitionKey="${tool_api}" RowKey="${GUID}" NoOfClientsStarted=$no_of_clients_started --if-match=$etag)
...
if [ $MACHINE_INDEX -eq 1 ]; then
  ...
  # Handle URL construction based on authentication mode
  if [[ $USE_MANAGED_IDENTITY_FOR_STORAGE == true ]]; then
    # For managed identity, append /* to the base URL
    new_storage_url="${result_storage_url}/*"
  else
    # For SAS token, insert /* before the ?
  ...
  az storage entity merge --table-name "${benchmarkname}Metadata" $AUTH_MODE $ACCOUNT_NAME_PARAM --entity PartitionKey="${tool_api}" RowKey="${GUID}" JobFinishTime=$finish_time JobStatus="Finished" NoOfClientsCompleted=$no_of_clients_completed --if-match=$etag
  ...
  for j in $(seq 1 60); do
    ...
    no_of_clients_completed=$((no_of_clients_completed + 1))
    echo "Updating latest table entry with incremented NoOfClientsCompleted"
    replace_entry_result=$(az storage entity merge --table-name "${benchmarkname}Metadata" $AUTH_MODE $ACCOUNT_NAME_PARAM --entity PartitionKey="${tool_api}" RowKey="${GUID}" NoOfClientsCompleted=$no_of_clients_completed --if-match=$etag)
    if [ -z "$replace_entry_result" ]; then
