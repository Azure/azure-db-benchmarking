...
- <span style="color:red">**Warning**</span>: 
    - This tool disrupts the network traffic between the specified target VM(s)/VMSS and the specified Cosmos DB collection/container for the specified account within the specified region.
        - If public access is enabled for the Cosmos DB account, the tool can potentially affect multiple Cosmos DB accounts in the region. 
        - If only private endpoint is enabled while the public access is disabled, then, the network disruption will only affect other collections in the account. 
    - The tool may also disrupt network traffic for other applications running on the specified target VM(s)/VMSS.
